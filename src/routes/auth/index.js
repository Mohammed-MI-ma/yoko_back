...
const limiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutesm
  max: 10, // limit each IP to 100 requests per windowMs
  handler: (req, res) => {
    res
      .status(429)
      .json({ error: "Too many requests, please try again later in 15 min" });
  },
});

router.post("/findUserByEmail", limiter, isUserFoundByEmail);

// Create a rate limiter for one request per minute
const limiterOTP = rateLimit({
  windowMs: 60 * 1000, // 1 minute
  max: 1, // limit each IP to 1 request per windowMs
  handler: (req, res) => {
    const remainingTime = humanizeMs(req.rateLimit.resetTime - Date.now(), {
      terse: true,
    });

    res.status(429).json({
      timeRemaining: remainingTime,
      error: `Too many requests, please try again in ${remainingTime}.`,
    });
  },
});
